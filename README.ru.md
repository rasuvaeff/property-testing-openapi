...
- PHP 8.3 – 8.5
- `ext-mbstring`
- `rasuvaeff/openapi-contract` ^0.13 и `rasuvaeff/property-testing-core` ^1.0
...
с тем же regex, который компилирует validation backend. Witness остаётся
внутри окна `minLength`/`maxLength`, а ошибка PCRE или исчерпанный бюджет
кандидатов/времени отвергаются fail-closed, без угадывания.

Каждая категория значений *проверяется*, а не заявляется. Кандидат-witness
оставляется, только если схема свойства его отвергает, а та же схема без
ключевых слов категории — принимает; проверка идёт через
`Contract::accepts()`, то есть witness судят те же правила, которые будут
судить сгенерированный запрос. Категория, у которой ни один кандидат не прошёл
проверку, для этой цели неконструируема — и операция просто не получает случая
такого вида вместо случая, чьё имя врёт о том, какое утверждение отвергло
запрос.

Два следствия стоит знать. `format` рядом с границей
`minLength`/`maxLength` теперь даёт случай всякий раз, когда фиксированный
witness помещается в окно, — самая частая строковая форма в реальном документе
раньше не получала ни `format`-случая, ни `length`. И witness, который
нарушил бы второе ключевое слово, отвергается даже там, где в схеме ничего не
было записано как конфликтующее: строка-маркер не может противоречить `enum`
из целых чисел, не противореча заодно типу, — поэтому категории `enum` и
`const` предлагают типизированные альтернативы, а проверка выбирает ту, что
кусает ровно один раз.
...
свойства перебираются в порядке объявления; `not`-схемы и объединения
типов пропускаются (свойство с OAS 3.0 `nullable` сохраняет кейсы, которые
дают его остальные ключевые слова — `null` допущен в дополнение к ним, а не
вместо); body, объявленное под несколькими media type, мутируется только в
JSON-варианте. Вложенные свойства пока не
